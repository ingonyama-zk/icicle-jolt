...
namespace msm_g2 {
  ...
  namespace {
    __global__ void
    precompute_points_kernel(const bn254::g2_affine_t* points, int shift, int prec_factor, int count, bn254::g2_affine_t* points_out, bool is_montgomery)
    {
      int tid = blockIdx.x * blockDim.x + threadIdx.x;
      if (tid >= count) return;
      bn254::g2_affine_t point_a = is_montgomery ? bn254::g2_affine_t::from_montgomery(points[tid]) : points[tid];
      bn254::g2_projective_t point = bn254::g2_projective_t::from_affine(point_a);
      points_out[tid * prec_factor] = bn254::g2_projective_t::to_affine(point);
      for (int i = 1; i < prec_factor; i++) {
        for (unsigned j = 0; j < shift; j++)
          point = bn254::g2_projective_t::dbl(point);
        points_out[tid * prec_factor + i] = bn254::g2_projective_t::to_affine(point);
      }
    }
    ...
    __global__ void single_stage_multi_reduction_kernel(
      const bn254::g2_projective_t* v,
      bn254::g2_projective_t* v_r,
    ...
    __global__ void initialize_buckets_kernel(bn254::g2_projective_t* buckets, unsigned N)
    {
      unsigned tid = (blockIdx.x * blockDim.x) + threadIdx.x;
      if (tid < N) buckets[tid] = bn254::g2_projective_t::zero(); // zero point
    }
      ...
      unsigned nof_precomputed_bms_per_msm)
    {
      ...
      const bn254::scalar_t& scalar = scalars[tid];
    ...
    __global__ void accumulate_buckets_kernel(
      bn254::g2_projective_t* __restrict__ buckets,
      ...
      const bn254::g2_affine_t* __restrict__ points,
      ...
      bool init_buckets)
    {
      ...
      for (unsigned i = 0; i < bucket_size;
        ...
        bn254::g2_affine_t point = points[point_ind];
        bucket = i || !init_buckets ? (point == bn254::g2_affine_t::zero() ? bucket : bucket + point)
                                    : (point == bn254::g2_affine_t::zero() ? bn254::g2_projective_t::zero() : bn254::g2_projective_t::from_affine(point));
      }
      ...
      const unsigned nof_threads)
    {
      ...
      for (unsigned i = 0; i < run_length;
        ...
        bn254::g2_affine_t point = points[point_ind];
        bucket =
          i ? (point == bn254::g2_affine_t::zero() ? bucket : bucket + point) : (point == bn254::g2_affine_t::zero() ? bn254::g2_projective_t::zero() : bn254::g2_projective_t::from_affine(point));
      }
      buckets[tid] = run_length ? bucket : bn254::g2_projective_t::zero();
    }

    __global__ void distribute_large_buckets_kernel(
      const bn254::g2_projective_t* large_buckets,
      bn254::g2_projective_t* buckets,
    ...
    __global__ void big_triangle_sum_kernel(const bn254::g2_projective_t* buckets, bn254::g2_projective_t* final_sums, unsigned nof_bms, unsigned c)
    {
      unsigned tid = (blockIdx.x * blockDim.x) + threadIdx.x;
      if (tid >= nof_bms) return;
      unsigned buckets_in_bm = (1 << c);
      bn254::g2_projective_t line_sum = buckets[(tid + 1) * buckets_in_bm - 1];
    ...
    __global__ void last_pass_kernel(
      const bn254::g2_projective_t* final_buckets,
      bn254::g2_projective_t* final_sums,
    ...
    __global__ void final_accumulation_kernel(
      const bn254::g2_projective_t* final_sums, bn254::g2_projective_t* final_results, unsigned nof_msms, unsigned nof_results, unsigned c)
    {
      unsigned tid = (blockIdx.x * blockDim.x) + threadIdx.x;
      if (tid >= nof_msms) return;
      bn254::g2_projective_t final_result = bn254::g2_projective_t::zero();
    ...
    static cudaError_t split_and_sort_scalars(
      ...
      const bn254::scalar_t* d_scalars,
      unsigned nof_points,
    ...
    static cudaError_t upload_scalars(
      ...
      unsigned nof_scalars,
      const bn254::scalar_t* scalars,
      bn254::scalar_t*& d_allocated_scalars,
      const bn254::scalar_t*& d_scalars)
    {
    ...
    static cudaError_t upload_points(
      ...
      bn254::g2_affine_t** points_processed,
      unsigned nof_points,
      bn254::g2_affine_t*& d_allocated_points,
      const bn254::g2_affine_t*& d_points)
    {
      if (init_points) {
        if (!are_points_on_device || are_points_montgomery_form) CHK_IF_RETURN(cudaStreamCreate(&stream_points));
        if (!are_points_on_device) {
          // copy points to gpu
          CHK_IF_RETURN(cudaMallocAsync(&d_allocated_points, sizeof(bn254::g2_affine_t) * nof_points, stream_points));
          CHK_IF_RETURN(
            cudaMemcpyAsync(d_allocated_points, points, sizeof(bn254::g2_affine_t) * nof_points, cudaMemcpyHostToDevice, stream_points));
    ...
    static cudaError_t allocate_and_sort_buckets_by_size(
      cudaStream_t stream,
      bool init_buckets,
      bn254::g2_projective_t*& buckets, /*out*/
      bn254::g2_projective_t** buckets_external,
      unsigned total_nof_buckets,
    ...
    {
      if (init_buckets) {
        CHK_IF_RETURN(cudaMallocAsync(&buckets, sizeof(bn254::g2_projective_t) * (total_nof_buckets + nof_bms_in_batch), stream));
      ...
      bool init_buckets)
    {
      ...
      initialize_large_bucket_indices<<<NUM_BLOCKS, NUM_THREADS, 0, stream_large_buckets>>>(
        d_sorted_bucket_sizes_sum, average_bucket_size, h_nof_large_buckets, log_nof_large_buckets,
        large_bucket_indices);

      bn254::g2_projective_t* large_buckets;
      CHK_IF_RETURN(cudaMallocAsync(&large_buckets, sizeof(bn254::g2_projective_t) * large_buckets_nof_threads, stream_large_buckets));
      ...
      cudaEvent_t& event_large_buckets_accumulated)
    {
      ...
      if (h_nof_large_buckets > 0 && bucket_th > 0) {
        auto err = large_bucket_accumulation(
    ...
    static cudaError_t bucket_reduction(
      ...
      bn254::g2_projective_t* final_result,
      bn254::g2_projective_t* d_allocated_final_result,
      bool _is_big_triangle)
    {
      ...
      bn254::g2_projective_t* final_results;
      if (_is_big_triangle || c == 1) {
        CHK_IF_RETURN(cudaMallocAsync(&final_results, sizeof(bn254::g2_projective_t) * nof_bms_in_batch, stream));
        ...
        bn254::g2_projective_t* source_buckets = buckets;
        buckets = nullptr;
        bn254::g2_projective_t* target_buckets;
        bn254::g2_projective_t* temp_buckets1;
        bn254::g2_projective_t* temp_buckets2;
        for (unsigned i = 0;; i++) {
          ...
          CHK_IF_RETURN(cudaMallocAsync(&target_buckets, sizeof(bn254::g2_projective_t) * target_buckets_count * batch_size, stream));
          CHK_IF_RETURN(cudaMallocAsync(
            &temp_buckets1, sizeof(bn254::g2_projective_t) * source_buckets_count * batch_size,
            stream)); // for type1 reduction (strided, bottom window - evens)
          CHK_IF_RETURN(cudaMallocAsync(
            &temp_buckets2, sizeof(bn254::g2_projective_t) * source_buckets_count * batch_size,
          ...
          if (target_bits_count == 1) {
            ...
            CHK_IF_RETURN(cudaMallocAsync(&final_results, sizeof(bn254::g2_projective_t) * total_nof_final_results, stream));
      ...
      final_accumulation_kernel<<<NUM_BLOCKS, NUM_THREADS, 0, stream>>>(
      ...
      cudaStream_t stream)
    {
      ...
      bn254::g2_affine_t* d_allocated_points = nullptr;
      const bn254::g2_affine_t* d_points = nullptr;

      bn254::scalar_t* d_allocated_scalars = nullptr;
      const bn254::scalar_t* d_scalars = nullptr;

      cudaEvent_t event_points_uploaded = nullptr;
      bn254::g2_projective_t* buckets = nullptr;
      ...
      CHK_IF_RETURN((allocate_and_sort_buckets_by_size(
      ...
      CHK_IF_RETURN((compute_nof_large_buckets(
      ...
      CHK_IF_RETURN((bucket_accumulation(
      ...
      bn254::g2_projective_t* d_allocated_final_result = nullptr;
      if (!are_results_on_device)
        CHK_IF_RETURN(cudaMallocAsync(&d_allocated_final_result, sizeof(bn254::g2_projective_t) * batch_size, stream));

      // (6) Bucket Reduction - sum each bucket module then finally sum all buckets module to final sum
      CHK_IF_RETURN((bucket_reduction(
      ...
      if (!are_results_on_device)
        CHK_IF_RETURN(cudaMemcpyAsync(
          final_result, d_allocated_final_result, sizeof(bn254::g2_projective_t) * batch_size, cudaMemcpyDeviceToHost, stream));
    ...
    double& reduced_gpu_memory)
  {
    ...
    scalars_mem = sizeof(bn254::scalar_t) * msm_size * batch_size;
    ...
    points_mem = sizeof(bn254::g2_affine_t) * msm_size * config.precompute_factor * (config.are_points_shared_in_batch ? 1 : batch_size);
    buckets_mem = 4 * sizeof(bn254::g2_projective_t) * (1 << c) * batch_size *
      ...
      if (buckets_mem > reduced_gpu_memory) {
        ...
        while (buckets_mem > reduced_gpu_memory) {
          fixed_c = floor(std::log2(
            ...
            static_cast<double>(
              3 * sizeof(bn254::g2_projective_t) * nof_bms_after_precomputation))); // nof_bms_after_precomputation is a function of c so
  ...
  static int get_optimal_nof_chunks(
    const MSMConfig& config, int msm_size, unsigned c, unsigned bitsize, unsigned& fixed_c, int& fixed_batch_size)
  {
    int min_nof_chunks = get_min_nof_chunks(config, msm_size, c, bitsize, fixed_c, fixed_batch_size);
  ...
  */
    ...
    cudaStream_t stream)
  {
    ...
    bn254::g2_affine_t* points_d;
    bn254::g2_affine_t* points_processed;
    bn254::scalar_t* scalars_d;
    bn254::g2_projective_t* buckets_d;
    if ((!config.are_scalars_on_device) && nof_chunks > 1) {
      CHK_IF_RETURN(cudaMallocAsync(&scalars_d, sizeof(bn254::scalar_t) * scalars_chunk_size * 2, stream));
    }
    if (((!config.are_points_on_device)) && nof_chunks > 1) {
      CHK_IF_RETURN(cudaMallocAsync(&points_d, sizeof(bn254::g2_affine_t) * points_chunk_size * 2, stream));
    }
    ...
    for (int i = 0; i < nof_chunks; i++) {
      ...
      if (!config.are_scalars_on_device)
        CHK_IF_RETURN(cudaMemcpyAsync(
          ...
          sizeof(bn254::scalar_t) * sub_scalars_size, cudaMemcpyHostToDevice, transfer_stream)); // scalars on host
      if (!config.are_points_on_device && !batch_same_points)
        CHK_IF_RETURN(cudaMemcpyAsync(
          points_d + ((i + 1) % 2) * points_chunk_size, points + (i + 1) * points_chunk_size,
          sizeof(bn254::g2_affine_t) * sub_points_size, cudaMemcpyHostToDevice, transfer_stream)); // points on host
    }
    // sync with main stream
  ...
  cudaError_t msm_cuda(const bn254::scalar_t* scalars, const bn254::g2_affine_t* points, int msm_size, const MSMConfig& config, bn254::g2_projective_t* results)
  {
    ...
    if (_nof_chunks) {
      ...
      min_nof_chunks = get_min_nof_chunks(config, msm_size, c, bitsize, fixed_c, fixed_batch_size);
      ...
      nof_chunks = get_optimal_nof_chunks(config, msm_size, c, bitsize, fixed_c, fixed_batch_size);
    }
  ...
  static cudaError_t precompute_msm_points_chunk(
    ...
    bool are_points_montgomery_form,
    cudaStream_t stream,
    bn254::g2_affine_t* output_points)
  {
    CHK_INIT_IF_RETURN();

    bn254::g2_affine_t* points_d;
    if (!are_points_on_device) {
      CHK_IF_RETURN(cudaMallocAsync(&points_d, sizeof(bn254::g2_affine_t) * points_size, stream));
      CHK_IF_RETURN(cudaMemcpyAsync(points_d, points, sizeof(bn254::g2_affine_t) * points_size, cudaMemcpyHostToDevice, stream));
    }

    unsigned total_nof_bms = (bn254::g2_projective_t::SCALAR_FF_NBITS - 1) / c + 1;
    unsigned shift = c * ((total_nof_bms - 1) / precompute_factor + 1);

    unsigned NUM_THREADS = 1 << 8;
    unsigned NUM_BLOCKS = (points_size + NUM_THREADS - 1) / NUM_THREADS;
    precompute_points_kernel<<<NUM_BLOCKS, NUM_THREADS, 0, stream>>>(
  ...
  static cudaError_t chunked_precompute(
    ...
    const MSMConfig& config,
    bn254::g2_affine_t* points_precomputed,
    int nof_chunks)
  {
    cudaStream_t stream = reinterpret_cast<cudaStream_t>(config.stream);
    bn254::g2_affine_t* points_d;
    bn254::g2_affine_t *points_precomputed_d, *points_precomputed_h;
    ...
    if (config.are_points_on_device) {
      ...
      CHK_IF_RETURN(cudaMallocAsync(&points_d, sizeof(bn254::g2_affine_t) * chunk_size * 2, stream));
      CHK_IF_RETURN(
        cudaMallocAsync(&points_precomputed_d, sizeof(bn254::g2_affine_t) * chunk_size * 2 * config.precompute_factor, stream));
    ...
    for (int i = 0; i < nof_chunks; i++) {
      ...
      if (!config.are_points_on_device) {
        CHK_IF_RETURN(cudaMemcpyAsync(
          points_d + (i % 2) * chunk_size, points + i * chunk_size, sizeof(bn254::g2_affine_t) * sub_msm_size, cudaMemcpyHostToDevice,
        ...
        CHK_IF_RETURN(cudaMemcpyAsync(
          ...
          points_precomputed_d + (i % 2) * chunk_size * config.precompute_factor,
          sizeof(bn254::g2_affine_t) * sub_msm_size * config.precompute_factor, cudaMemcpyDeviceToHost, stream));
      } else {
        CHK_IF_RETURN((precompute_msm_points_chunk(
  ...
  {
    unsigned long points_mem = sizeof(bn254::g2_affine_t) * msm_size * config.precompute_factor * points_batch_size;
    unsigned long total_memory;
    unsigned long gpu_memory;
    cudaMemGetInfo(&gpu_memory, &total_memory); // TODO - handle cuda error
    double reduced_gpu_memory =
  ...
  static cudaError_t
  cuda_precompute_msm_points(const bn254::g2_affine_t* points, int msm_size, const MSMConfig& config, bn254::g2_affine_t* output_points)
  {
    ...
    nof_chunks = get_precomputation_nof_chunks(config, msm_size, 1);
    for (int i = 0; i < config.batch_size; i++) {
      CHK_IF_RETURN((chunked_precompute(
  ...
  static eIcicleError msm_cuda_wrapper_g2(
    const Device& device, const bn254::scalar_t* scalars, const bn254::g2_affine_t* bases, int msm_size, const MSMConfig& config, bn254::g2_projective_t* results)
  {
    auto err = msm_cuda(scalars, bases, msm_size, config, results);
    return translateCudaError(err);
  }

  static eIcicleError msm_precompute_bases_cuda_wrapper_g2(
    const Device& device, const bn254::g2_affine_t* input_bases, int nof_bases, const MSMConfig& config, bn254::g2_affine_t* output_bases)
  {
...
using namespace msm_g2;

// Note: splitting from cuda_msm.cu to compile it in parallel to g1 msm
REGISTER_MSM_G2_BACKEND("CUDA", (msm_cuda_wrapper_g2));
REGISTER_MSM_G2_PRE_COMPUTE_BASES_BACKEND(
  "CUDA", (msm_precompute_bases_cuda_wrapper_g2));