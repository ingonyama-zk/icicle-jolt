...
namespace msm_g2 {
  ...
  namespace {
      ...
      bool init_buckets)
    {
      ...
      for (unsigned i = 0; i < bucket_size;
        ...
        bucket = i || !init_buckets
                   ? (point == bn254::g2_affine_t::zero() ? bucket : bucket + point)
                   : (point == bn254::g2_affine_t::zero() ? bn254::g2_projective_t::zero()
                                                          : bn254::g2_projective_t::from_affine(point));
      }
      ...
      const unsigned nof_threads)
    {
      ...
      for (unsigned i = 0; i < run_length;
        ...
        unsigned point_ind = point_indices[bucket_offset + i];
        bn254::g2_affine_t point = points[point_ind];
        bucket = i ? (point == bn254::g2_affine_t::zero() ? bucket : bucket + point)
                   : (point == bn254::g2_affine_t::zero() ? bn254::g2_projective_t::zero()
                                                          : bn254::g2_projective_t::from_affine(point));
      }
    ...
    __global__ void big_triangle_sum_kernel(
      const bn254::g2_projective_t* buckets, bn254::g2_projective_t* final_sums, unsigned nof_bms, unsigned c)
    {
      unsigned tid = (blockIdx.x * blockDim.x) + threadIdx.x;
    ...
    __global__ void final_accumulation_kernel(
      const bn254::g2_projective_t* final_sums,
      ...
      const bn254::g2_affine_t*& d_points)
    {
      if (init_points) {
        ...
        if (!are_points_on_device) {
          ...
          CHK_IF_RETURN(cudaMemcpyAsync(
            d_allocated_points, points, sizeof(bn254::g2_affine_t) * nof_points, cudaMemcpyHostToDevice,
            stream_points));
      ...
      unsigned* d_bucket_offsets)
    {
      if (init_buckets) {
        CHK_IF_RETURN(
          cudaMallocAsync(&buckets, sizeof(bn254::g2_projective_t) * (total_nof_buckets + nof_bms_in_batch), stream));
      ...
      bool init_buckets)
    {
      ...
      CHK_IF_RETURN(cudaMallocAsync(
        &large_buckets, sizeof(bn254::g2_projective_t) * large_buckets_nof_threads, stream_large_buckets));
      ...
      bool _is_big_triangle)
    {
      ...
      if (_is_big_triangle || c == 1) {
        ...
        for (unsigned i = 0;; i++) {
          ...
          CHK_IF_RETURN(cudaMallocAsync(
            &target_buckets, sizeof(bn254::g2_projective_t) * target_buckets_count * batch_size, stream));
          CHK_IF_RETURN(cudaMallocAsync(
          ...
          if (target_bits_count == 1) {
            ...
            CHK_IF_RETURN(
              cudaMallocAsync(&final_results, sizeof(bn254::g2_projective_t) * total_nof_final_results, stream));
      ...
      cudaStream_t stream)
    {
      ...
      if (!are_results_on_device)
        CHK_IF_RETURN(cudaMemcpyAsync(
          final_result, d_allocated_final_result, sizeof(bn254::g2_projective_t) * batch_size, cudaMemcpyDeviceToHost,
          stream));
    ...
    double& reduced_gpu_memory)
  {
    ...
    points_mem = sizeof(bn254::g2_affine_t) * msm_size * config.precompute_factor *
      ...
      if (buckets_mem > reduced_gpu_memory) {
        ...
        while (buckets_mem > reduced_gpu_memory) {
          fixed_c = floor(std::log2(
            ...
            static_cast<double>(
              3 * sizeof(bn254::g2_projective_t) *
  ...
  */
    ...
    bn254::g2_affine_t* output_points)
  {
    ...
    if (!are_points_on_device) {
      ...
      CHK_IF_RETURN(
        cudaMemcpyAsync(points_d, points, sizeof(bn254::g2_affine_t) * points_size, cudaMemcpyHostToDevice, stream));
    }
    ...
    int nof_chunks)
  {
    ...
    if (config.are_points_on_device) {
      ...
      CHK_IF_RETURN(cudaMallocAsync(
        &points_precomputed_d, sizeof(bn254::g2_affine_t) * chunk_size * 2 * config.precompute_factor, stream));
    ...
    for (int i = 0; i < nof_chunks; i++) {
      ...
      if (!config.are_points_on_device) {
        CHK_IF_RETURN(cudaMemcpyAsync(
          points_d + (i % 2) * chunk_size, points + i * chunk_size, sizeof(bn254::g2_affine_t) * sub_msm_size,
  ...
  static cudaError_t cuda_precompute_msm_points(
    const bn254::g2_affine_t* points, int msm_size, const MSMConfig& config, bn254::g2_affine_t* output_points)
  {
  ...
  static eIcicleError msm_precompute_bases_cuda_wrapper_g2(
    const Device& device,
    const bn254::g2_affine_t* input_bases,
    int nof_bases,
    const MSMConfig& config,
...
REGISTER_MSM_G2_PRE_COMPUTE_BASES_BACKEND("CUDA", (msm_precompute_bases_cuda_wrapper_g2));