...
namespace msm {
  ...
  namespace {
      ...
      bool is_montgomery)
    {
      ...
      if (tid >= count) return;
      bn254::affine_t point_a = is_montgomery ? bn254::affine_t::from_montgomery(points[tid]) : points[tid];
      bn254::projective_t point = bn254::projective_t::from_affine(point_a);
      points_out[tid * prec_factor] = bn254::projective_t::to_affine(point);
      for (int i = 1; i < prec_factor; i++) {
        for (unsigned j = 0; j < shift; j++)
          point = bn254::projective_t::dbl(point);
        points_out[tid * prec_factor + i] = bn254::projective_t::to_affine(point);
      }
    }
    ...
    __global__ void single_stage_multi_reduction_kernel(
      const bn254::projective_t* v,
      bn254::projective_t* v_r,
    ...
    __global__ void initialize_buckets_kernel(bn254::projective_t* buckets, unsigned N)
    {
      unsigned tid = (blockIdx.x * blockDim.x) + threadIdx.x;
      if (tid < N) buckets[tid] = bn254::projective_t::zero(); // zero point
    }
    ...
    __global__ void accumulate_buckets_kernel(
      bn254::projective_t* __restrict__ buckets,
      ...
      const bn254::affine_t* __restrict__ points,
    ...
    __global__ void distribute_large_buckets_kernel(
      const bn254::projective_t* large_buckets,
      bn254::projective_t* buckets,
      ...
      const bn254::projective_t* buckets, bn254::projective_t* final_sums, unsigned nof_bms, unsigned c)
    {
      ...
      bn254::projective_t line_sum = buckets[(tid + 1) * buckets_in_bm - 1];
    ...
    __global__ void last_pass_kernel(
      const bn254::projective_t* final_buckets,
      bn254::projective_t* final_sums,
      ...
      unsigned c)
    {
      ...
      bn254::projective_t final_result = bn254::projective_t::zero();
    ...
    static cudaError_t upload_points(
      ...
      const bn254::affine_t* points,
      bn254::affine_t** points_processed,
      unsigned nof_points,
      bn254::affine_t*& d_allocated_points,
      const bn254::affine_t*& d_points)
    {
      if (init_points) {
    ...
    static cudaError_t allocate_and_sort_buckets_by_size(
      cudaStream_t stream,
      bool init_buckets,
      bn254::projective_t*& buckets, /*out*/
      bn254::projective_t** buckets_external,
      unsigned total_nof_buckets,
    ...
    static cudaError_t bucket_reduction(
      ...
      bn254::projective_t* final_result,
      bn254::projective_t* d_allocated_final_result,
      bool _is_big_triangle)
    {
      ...
      if (_is_big_triangle || c == 1) {
        CHK_IF_RETURN(cudaMallocAsync(&final_results, sizeof(bn254::projective_t) * nof_bms_in_batch, stream));
        ...
        bn254::projective_t* source_buckets = buckets;
        buckets = nullptr;
        bn254::projective_t* target_buckets;
        bn254::projective_t* temp_buckets1;
        bn254::projective_t* temp_buckets2;
        for (unsigned i = 0;; i++) {
          ...
          CHK_IF_RETURN(cudaMallocAsync(
            &temp_buckets2, sizeof(bn254::projective_t) * source_buckets_count * batch_size,
      ...
      cudaStream_t stream)
    {
      ...
      bn254::affine_t* d_allocated_points = nullptr;
      const bn254::affine_t* d_points = nullptr;
      ...
      bn254::projective_t* buckets = nullptr;
      ...
      bn254::projective_t* d_allocated_final_result = nullptr;
      if (!are_results_on_device)
        CHK_IF_RETURN(cudaMallocAsync(&d_allocated_final_result, sizeof(bn254::projective_t) * batch_size, stream));
  ...
  */
    ...
    cudaStream_t stream)
  {
    ...
    if (((!config.are_points_on_device)) && nof_chunks > 1) {
      CHK_IF_RETURN(cudaMallocAsync(&points_d, sizeof(bn254::affine_t) * points_chunk_size * 2, stream));
    }
    ...
    for (int i = 0; i < nof_chunks; i++) {
      ...
      if (!config.are_points_on_device && !batch_same_points)
        CHK_IF_RETURN(cudaMemcpyAsync(
          ...
          sizeof(bn254::affine_t) * sub_points_size, cudaMemcpyHostToDevice, transfer_stream)); // points on host
    }
    // sync with main stream
  ...
  static cudaError_t chunked_precompute(
    ...
    const MSMConfig& config,
    bn254::affine_t* points_precomputed,
    int nof_chunks)
  {
    cudaStream_t stream = reinterpret_cast<cudaStream_t>(config.stream);
    bn254::affine_t* points_d;
    bn254::affine_t *points_precomputed_d, *points_precomputed_h;
    ...
    for (int i = 0; i < nof_chunks; i++) {
      ...
      if (!config.are_points_on_device) {
        ...
        CHK_IF_RETURN(cudaMemcpyAsync(
          ...
          points_precomputed_d + (i % 2) * chunk_size * config.precompute_factor,
          sizeof(bn254::affine_t) * sub_msm_size * config.precompute_factor, cudaMemcpyDeviceToHost, stream));
      } else {
        CHK_IF_RETURN((precompute_msm_points_chunk(
  ...
  {
    unsigned long points_mem = sizeof(bn254::affine_t) * msm_size * config.precompute_factor * points_batch_size;
    unsigned long total_memory;
    unsigned long gpu_memory;
    cudaMemGetInfo(&gpu_memory, &total_memory); // TODO - handle cuda error
    double reduced_gpu_memory =
...
REGISTER_MSM_BACKEND("CUDA", (msm_cuda_wrapper));
REGISTER_MSM_PRE_COMPUTE_BASES_BACKEND("CUDA", (msm_precompute_bases_cuda_wrapper));